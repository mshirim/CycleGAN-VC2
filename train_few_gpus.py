    ...
    def train(self, rank, world_size):
        ...
        for epoch in range(self.start_epoch, self.num_epochs):
            ...
            dataset = trainingDataset(datasetA=self.dataset_A,
                                      ...
                                      n_frames=128)
            # I added #
            self.generator_A2B.to(f'cuda:{rank}')
            self.generator_A2B = torch.nn.parallel.DistributedDataParallel(self.generator_A2B, device_ids=[rank], find_unused_parameters=True)
            self.generator_B2A.to(f'cuda:{rank}')
            self.generator_B2A = torch.nn.parallel.DistributedDataParallel(self.generator_B2A, device_ids=[rank], find_unused_parameters=True)
            self.discriminator_A.to(f'cuda:{rank}')
            self.discriminator_A = torch.nn.parallel.DistributedDataParallel(self.discriminator_A, device_ids=[rank], find_unused_parameters=True)
            self.discriminator_B.to(f'cuda:{rank}')
            self.discriminator_B = torch.nn.parallel.DistributedDataParallel(self.discriminator_B, device_ids=[rank], find_unused_parameters=True)
            ...
            for i, (real_A, real_B) in enumerate(train_loader):
                ...
                cycleLoss = torch.mean(
                    torch.abs(real_A.to(f'cuda:{rank}') - cycle_A.to(f'cuda:{rank}'))) + \
                            torch.mean(torch.abs(real_B.to(f'cuda:{rank}') - cycle_B.to(f'cuda:{rank}')))

                # Generator Identity Loss
                identiyLoss = torch.mean(
                    torch.abs(real_A.to(f'cuda:{rank}') - identity_A.to(f'cuda:{rank}'))) + \
                              torch.mean(torch.abs(real_B.to(f'cuda:{rank}') - identity_B.to(f'cuda:{rank}')))
                ...
                generator_loss = generator_loss_A2B.to('cuda:0') + generator_loss_B2A.to('cuda:0') + \
                                 cycle_loss_lambda * cycleLoss.to('cuda:0') + \
                                 identity_loss_lambda * identiyLoss.to('cuda:0')
...
def run_train(rank, world_size, cycleGAN):
    cycleGAN.train(rank, world_size)
...
if __name__ == '__main__':
    ...
    validation_A_dir_default = place + '/data/jane/'
    output_A_dir_default = place + '/converted_sound/jane'

    validation_B_dir_default = place + '/data/teller/'
    output_B_dir_default = place + '/converted_sound/teller/'
    ...
    #CycleGANTraining.pre_run()
    world_size = 4
    torch.multiprocessing.spawn(run_train, args=(world_size, cycleGAN),
                                nprocs=world_size)
    #cycleGAN.train()
